...
class box_ploter:
    ...
    def dot_box_plot(self,df,x,y,out,hue=False,ylabel='',xlabel='',
                     label_median=True,showmeans=True,
                     showfliers=False,figsize = (8, 8)):
        """
        ...
        params = dict(data=df,
                      x=x,y=y,hue=hue,
                      dodge=True)
        sns.stripplot( size=8,
                          jitter=0.35,
                          palette='bright',
                          edgecolor='black',
                          linewidth=1,
                          **params)
        p_box = sns.boxplot(palette='pastel',linewidth=4, showmeans=showmeans,
                            showfliers=False,**params)
        if label_median :
            ...
            for xtick in p_box.get_xticks():
                p_box.text(xtick,maxes[xtick]+vertical_offset,round(medians[xtick],3), 
...
class line_ploter:
    ...
    def line_plot(self,df,x,y,err,out,labs=[[None,None]],ylabel='',xlabel='',
                     fnt_size=10,x_rotation=0,x_lim=(0,1),y_lim=(0,1),
                     figsize = (8, 8)):
        """
        ...
        for index_ in range(len(y)):
            ...
            err_=err[index_]
            x_=x
            sns.lineplot(data=df,x=x_, y=y_, color=colors_[index_],
            ...
            ax.fill_between(np.array(df[x_]), bound_low, bound_up, color=colors_[index_], alpha=.1,
