...
class FeatureSelector:
        ...
        for _ in range(n_iterations):
            ...
            if task == "classification":
                model = lgb.LGBMClassifier(
                    n_estimators=1000, learning_rate=0.025, n_jobs=2, verbose=-1
                )

            elif task == "regression":
                model = lgb.LGBMRegressor(
                    n_estimators=1000, learning_rate=0.025, verbose=-1
                )
            ...
            if early_stopping:
                ...
                (
                    ...
                    train_labels,
                    valid_labels,
                ) = train_test_split(features, labels, test_size=0.15, stratify=labels)
    ...
    def identify_zero_importance_bagging(
        self, task, eval_metric=None, n_iterations=10, early_stopping=True
    ):
        """
        ...
        if task == "classification":
            model = lgb.LGBMClassifier(
                n_estimators=100, learning_rate=0.025, verbose=-1
            )
        ...
        for iters_ in range(len(mod.estimators_)):
            for fits_ in range(len(mod.estimators_features_[iters_])):
                ...
                fitDict[feature_names[mod.estimators_features_[iters_][fits_]]][
                    1
                ] += fitIMPs[iters_][fits_]
        ...
        feature_importances = pd.DataFrame(
            {
                "feature": list(fitDictWei.keys()),
                "importance": list(fitDictWei.values()),
            }
        )
    ...
    def identify_zero_importance_ensemble(
        self, task, eval_metric=None, n_iterations=10, early_stopping=True
    ):
        """
        ...
        # shuffle the data for each iterations
        X, Y = shuffle(
            features, labels, random_state=123
        )  # bootstrapping is used instead
        ...
        mod = BaggingClassifier(
            rf, n_estimators=n_iterations, bootstrap=True, n_jobs=-1, random_state=123
        )
        mod.fit(X, Y)
        fitIMPs = [
            clf.feature_importances_ / clf.feature_importances_.sum()
        ...
        mod = BaggingClassifier(
            xg, n_estimators=n_iterations, bootstrap=True, n_jobs=-1, random_state=123
        )
        mod.fit(X, Y)
        fitIMPs = [
            clf.feature_importances_ / clf.feature_importances_.sum()
        ...
        feature_importance_values = (
            feature_importance_values / feature_importance_values.sum()
        )
    ...
    def plot_collinear(self, plot_all=False):
        ...
        ax.set_yticklabels(
            list(corr_matrix_plot.index), size=int(120 / corr_matrix_plot.shape[0])
        )
        # orignially 160
        ...
        ax.set_xticklabels(
            list(corr_matrix_plot.columns), size=int(120 / corr_matrix_plot.shape[1])
        )
    ...
    def identify_fit_importance_xgb(self, phenType="quant"):
        ...
        if phenType == "cc":
            mod = xgb.XGBClassifier(
                nthread=-1, random_state=1487
            )  # n_estimators=n_estimators, learning_rate = eta(we need default parameters)
            ...
            cvScore = cross_validate(
                mod, features_array, labels, n_jobs=-1, cv=5, scoring=scoring
            )
            accDf = pd.DataFrame(
                {
                    "expVar": cvScore["test_explained_variance"],
                    "r2": cvScore["test_r2"],
                    "rmse": cvScore["test_neg_mean_squared_error"],
                }
            )
        ...
        feature_importances = pd.DataFrame(
            {"feature": feature_names, "importance": mod.feature_importances_}
        )
        ...
        feature_importances.sort_values(
            by="normalized_importance", ascending=False, inplace=True
        )
    ...
    def identify_fit_importance_lr(self, phenType="quant"):
        ...
        feature_importances = pd.DataFrame(
            {"feature": feature_names, "importance": mod.coef_[0]}
        )  # coef_ returns ndarray of shape (1, n_features) or (n_classes, n_features)
        ...
        # Normalize the feature importances to add up to one
        feature_importances["normalized_importance"] = (
            np.absolute(feature_importances["importance"])
            / np.absolute(feature_importances["importance"]).sum()
        )
        feature_importances["cumulative_importance"] = np.cumsum(
            feature_importances["normalized_importance"]
        )
