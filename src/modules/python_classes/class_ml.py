...
class param_optimizer:
    ...
    def cv_folds(self, test_siz=0.2, typ="sscv", folds=20, random_state=123):
        ...
        if typ == "sscv":
            cvs = StratifiedShuffleSplit(
                n_splits=folds, test_size=test_siz, random_stat=random_state
            )
        return cvs
    ...
    def hyperopt_xgb(self, xgbpar):
        ...
        auc = cross_val_score(
            mdl, self.dfv, self.target, cv=self.xgb_cv, scoring="roc_auc"
        ).mean()
    ...
    def optimize_xgb(self, evals, optimizer=tpe.suggest, nthread=-1, random_state=123):
        ...
        spacexgb = {
            ...
            "eta": hp.quniform(
                "eta", 0.025, 0.25, 0.025
            ),  # A problem with max_depth casted to float instead of int with the hp.quniform method.
        ...
        print(
            "Best XGBClassfier ROCAUC score is equal to: {0:.3f}".format(
                self.xgb_best_score
            )
        )
        return best_param
...
class rocauc_estimate:
    ...
    def cv_folds(self, test_size=0.2, cv_type="sscv", folds=20, random_state=123):
        if cv_type == "sscv":
            cvs = StratifiedShuffleSplit(
                n_splits=folds, test_size=test_size, random_state=random_state
            )
        return cvs
    ...
    def hyperopt_rf(self, rfpar):
        ...
        auc = cross_val_score(
            mdl, self.x_train, self.y_train, cv=self.rf_cv, scoring="roc_auc", n_jobs=-1
        ).mean()
    ...
    def optimize_rf(
        self, evals, cv_fold=5, optimizer=tpe.suggest, nthread=-1, random_state=123
    ):
        spacerf = {
        ...
        trials = Trials()
        best_param = fmin(
            self.hyperopt_rf, spacerf, algo=tpe.suggest, max_evals=evals, trials=trials
        )
        best_param = space_eval(spacerf, best_param)
        print(
            "Best RandomForest ROCAUC score is equal to: {0:.3f}".format(
                self.rf_best_score
            )
        )
        return best_param
    ...
    def optimize_svc(
        ...
        spacesvc = {
            "C": hp.loguniform(
                "C", low=-2 * np.log(10), high=2 * np.log(10)
        ...
        print(
            "Best linearSVC ROCAUC score is equal to: {0:.3f}".format(
                self.svc_best_score
            )
        )
        return best_param
    ...
    def hyperopt_lr(self, lrpar):
        ...
        auc = cross_val_score(
            mdl, self.x_train, self.y_train, cv=self.lr_cv, scoring="roc_auc", n_jobs=-1
        ).mean()
    ...
    def optimize_lr(
        ...
        trials = Trials()
        best_param = fmin(
            self.hyperopt_lr, spacelr, algo=tpe.suggest, max_evals=evals, trials=trials
        )
        best_param = space_eval(spacelr, best_param)
        print(
            "Best logisticRegression ROCAUC score is equal to: {0:.3f}".format(
                self.lr_best_score
            )
        )
        return best_param
    ...
    def rocauc_estimator(
        ...
        hp_try=20,
        hp_cv_fold=3,
        which_hp={"lr": False, "rf": False, "xgb": False, "svc": True},
    ):
        ...
        cv = self.cv_folds(
            cv_type="sscv", folds=n_splits, test_size=test_size, random_state=123
        )
        ...
        for train_index, test_index in cv.split(X, Y):
            ...
            if hp_tune and which_hp["lr"]:
                ...
                best_param = self.optimize_lr(
                    evals=hp_try, optimizer=tpe.suggest, cv_fold=hp_cv_fold
                )
                lgr = self.lrmod
            else:
        ...
        ax.plot(
            ...
            mean_tpr_lgr,
            color="b",
            label=r"Logistic regression (AUC = %0.2f $\pm$ %0.2f)"
            % (mean_auc_lgr, std_auc_lgr),
            lw=2,
            alpha=0.8,
        )
    ...
    def permutation_score(
        ...
        plt.hist(
            permutation_scores,
            20,
            label="Permutation score =%s" % round(np.mean(permutation_scores), 3),
            edgecolor="black",
        )
        ...
        plt.plot(2 * [1.0 / 2], ylim, "--k", linewidth=3)  # , label='Luck'
